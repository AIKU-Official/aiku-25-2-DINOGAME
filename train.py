...
from config import OBSERVATION, GAMMA, BATCH_SIZE, LEARNING_RATE, REPLAY_MEMORY
...
def train_network(model, game_state, observe=False):
    ...
    D = deque(params["D"], maxlen = REPLAY_MEMORY) # 기존의 buffer를 불러오되, buffer size를 변경할 수도 있게 조정.
    t = params["time"]
    ...
    do_nothing = np.zeros(2) ##엎드리기안함
    do_nothing[0] = 1

    x_t, _,_ = game_state.get_state(do_nothing)
    s_t = np.stack((x_t, x_t, x_t, x_t), axis=2) # 프레임 스태킹 -> 안하는게 낫나? 고민중..
    ...
    while True:
        ...
        if random.random() <= epsilon:
            action_index = np.random.choice([0,1], p=[0.5, 0.5]) # ver.4: 엎드리는 경우는 의도적으로 낮게 sampling -> 엎드리기 안함
            a_t[action_index] = 1
            action_type = "explore"
        else:
        ...
        if t > OBSERVE:
            if t <= 100000:
                ...
                # 100k ~ 1M: 0.05 -> 0.05
                epsilon = 0.1
                # epsilon = 0.05 - (0.05 - 0.05) * ((t - 100000) / 900000)
            elif t <= 1500000:
                # 1M ~ 1.5M: 0.05 -> FINAL
                epsilon = 0.1 - (0.1 - FINAL_EPSILON) * ((t-1000000)/500000)
                # epsilon = 0.05 - (0.05 - FINAL_EPSILON) * ((t - 1000000) / 500000)
            else:
                # 이후는 FINAL로 고정
        ...
        if t > OBSERVE and t%4 == 0: # 4-step 마다 한 번만 학습!
            minibatch = random.sample(D, BATCH_SIZE)
            inputs = np.zeros((BATCH_SIZE, s_t.shape[1], s_t.shape[2], s_t.shape[3]))
            targets = np.zeros((BATCH_SIZE, 2)) ##엎드리기안함 
        ...
        # terminal 처리
        if terminal:
            print(f"[Episode finished] total reward: {round(episode_reward, 2)}")
            episode_reward = 0
